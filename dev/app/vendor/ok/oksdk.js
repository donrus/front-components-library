(function(global, factory) {
  ...
  function init(args, success, failure) {
    ...
    const params = getRequestParameters(args.location_search || window.location.search);
    const hParams = getRequestParameters(args.location_hash || window.location.hash);
    ...
    if (!params.api_server) {
      if (hParams.access_token == null && hParams.error == null) {
        window.location = `${state.widgetServer
        }oauth/authorize`
          + `?client_id=${
            args.app_id
          }&scope=${
  ...
  function restCall(method, params, callback, callOpts) {
    ...
    if (!callOpts || !callOpts.no_sig) {
      const secret = callOpts && callOpts.app_secret_key ? callOpts.app_secret_key : state.sessionSecretKey;
  ...
  function paymentShowInFrame(productName, productPrice, productCode, options, frameId) {
    const frameElement = `<iframe 'style='position: absolute; left: 0px; top: 0px; background-color: white; z-index: 9999;' src='${
  ...
  function injectAdsWidget(frameId, callbackFunction) {
    ...
    for (const prop in ads_widget_style) {
  ...
  function getAdsWidgetSrc() {
    ...
    const widgetSrc = `${state.widgetServer
  ...
  function isNativeAdSupported() {
    return (
      typeof OKApp !== 'undefined'
      && typeof OKApp.isAdsEnabled !== 'undefined'
      && OKApp.isAdsEnabled()
    );
  }
  ...
  function widgetOpen(widget, args, returnUrl) {
    ...
    let sigSource = '';
    let query = `${state.widgetServer}dk?st.cmd=${widget}&st.app=${state.app_id}`;
